...
/*	: select 문으로 데이터 조회 시, 원하는 데이터를 더 빠르게 찾아주는 도구
	- 데이터 검색 속도 향상 >> 전체 시스템 성능 높임
    
    EX) 책에서 원하는 단어를 찾을 떄 사용하는 '찾아보기(색인)'
    
    1. 인덱스 종류
    1) 클러스터형 인덱스
    : 기본키(PK) 설정 시 자동 생성
    - 테이블 당 단 하나만 생성 가능!
	- 자동 정렬 기능 포함: 지정된 열 (기본키 컬럼) 기준으로 자동 정렬
    select * from 'A';
    >> 위의 퀴문 처럼 order by(정렬)문이 없으면 클러스터형 인덱스를 기준으로 정렬
    
    EX) 영어사진, 국어사전: 내용이 미리 정렬
...
select * from members; -- 클리스터형 인덱스가 없는 경구(PK값이 없는 경우): 데이터 삽입 순서대로 출력
