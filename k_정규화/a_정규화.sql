...
/*	1. 정규화(Normalization) 개요
    ...
    cf) 정규형(Normal Form): 데이터베이스 정규화 과정에서 일정한 규칙이나 기준
    
    4. 정규화의 필요성
		id / name	/ coruse_id / course_name / course_instuctor
        1	  불멍이		101			슬립테크			바람둥이
        1	  불멍이		102			IoT				물렁이
        2    전기쟁이		102			IoT				물렁이
        
        >> 위 테이블의 문제점
			1) 중복데이터: 학생, 강의, 강사 정보가 중복 저장됨
            2) 이상 현상 발생
...
create table if not exists `student_course` (
    ...
    primary key (student_id, course_id) # student_id + sourse_id는 중복 허용 X / 비워질 수 없음
);
