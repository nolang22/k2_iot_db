...
WHERE grade NOT IN (
	...
	FROM `members`
    WHERE grade = 'Bronze');
# : NOT IN은 괄호 속 값들을 제회하라는 의미!
# >> 해당 쿼리는 Bronze를 먼저 뽑고, 해당 데이터를 제외하고 출력


SELECT GRADE
FROM `members`
WHERE grade IN ('Silver', 'Gold', 'Platinum', 'Diamond');
-- grade NOT IN('Bronze');
...
# - 정적인 조건 O (고정된 값)
# - 유연성 낮음
#		>> 'Bronze'외에 다른 등급이 싱기거나 이름이 바뀌면 수정
...
SELECT * FROM `purchases`; # 구매 ID(PK), 구매 회원 ID(FK), 상품 고드, 구매일, 가격 ,수량

# == 서브쿼리가 반드시 필요한 예시 == #
# 1) 구매한 이력이 있는 회원을 제외한 회원만 조회 (구매 이력이 없는 최원 조회)
...
SELECT 
	M.MEMBER_ID, M.NAME, T.TOTAL_AMONT
FROM `MEMBERS` M
	JOIN `PURCHASES` P
    (SUM(P.AMOUNT) * (SUM)P.QUANTITY) AS T
GROUP BY
	M.MEMBER_ID
ORDER BY
	T.TOTAL_AMONT DESC
