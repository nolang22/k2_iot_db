...
### DDL(Data Definittioni Language) 문법 정리 ###
...
-- CREATE CDATABASE example3  -- 다음 문장이 있는 경우 필수!
...
COLLATE = utf8mb4_unicode_ci; # 정렬 방식 (문자열까리 비교하고 정렬할 때 어떤 기준으로 할지 정하는 설정
# cf) ci (case-insensitive): 대소문자 구분하지 않음 ('abd' = 'ABC')
# cf) utf8mb4_general_ci: 속도가 빠름, 정확도 낮음

# utf8(문자까지만 인코딩) VS utf8mb4(모든 유니코드 문자 저장 - 이모지 & 한자포함)



# DB(스키마)를 지정하지 않으면 테이블 생성 불가!!!!!
# 1) 테이블명 앞에 'DB명.' 첨부
# 2) 위의 오휴 설명처럼 SCHEMAS 내에서 원하는 DB 더블 클릭
