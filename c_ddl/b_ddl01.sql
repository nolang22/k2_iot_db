...
# --- 데이터베이스 ---
# 1. 생성(CREATE)
create database database_name; -- 생성
drop database database_name; -- 삭제
# > 위 두개의 실행문은 각각 여러 번 실행할 경우 Error 발생

# cf) if [not] exists 옵션
# : 데이터베이스의 유무를 확인하고 오류를 반지하는 SQL문
#		, 존재할때 또는 존재하지 않을 때 실행

CREATE DATABASE IF NOT EXISTS database_name; # 존재하지 않을때 > 생성
DROP DATABASE IF EXISTS database_name; # 존재할 때 > 삭제

# 2. 선택(USE)
# : 선택 시 이후 모든 SQL 명령어가 선택된 DB 내에서 실행
...
# : 컬럼 수정 (modify)
# alter table 테이블명 modify colum 컬럼명 새로운컬럼_데이터타입 [기타사항];
alter table student
