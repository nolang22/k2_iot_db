...
# - '127.0.0.1': 해당 IP 주소를 가진 컴퓨터에서난 로그인 가능 (>> 127.0.0.1: localhost와 동일) 

# 2) 권한 부여
GRANT SELECT ON `korea_db`.* TO 'readonly_user'@'localhost';
GRANT SELECT ON `korea_db`.* TO 'readonly_user'@'127.0.0.1';
GRANT ALL PRIVILEGES ON `korea_db`.* TO 'readonly_user'@'127.0.0.1';
...
# CLI(명령어)로 로그인하는 방법 - 명령 프롬프트 창(cmd)
