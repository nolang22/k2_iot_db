...
# 2) 테이블 생성 시 마지막 열에 제약 조건 명시: PRIMATY KEY (설정할 컬럼명)
...
select * from `students`; # 테이블 내부의 살질적인 데이터 확인
...
desc `orders`;
# > MUL: 외래 키 지정 시 MySQl이 자동으로 인텍스(index) 생성
# 		- 외래 키 삭제 후에도 존재
