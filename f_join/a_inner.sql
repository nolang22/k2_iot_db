...
# cf) 조인 시 두개 이상의 테이블에서 동일한 열 이름이 존재하느 ㄴ경우
#		>> 테이블명(별칭).열이름 형식으로 표기!
...
# : 구매 이력이 있는 회원만 조회
SELECT
	DISTINCT M.NAME, M.CONTACT # 조인한 테이블으 ㅣ데이터가 반드시 조회될 필요는 없음
FROM
	`MEMBERS` M
    JOIN `PURCHASES` P
    ON M.MEMBER_ID = P.MEMBER_ID;

# cf) 서브쿼리 (구매 이역이 있는 회원 조회)
