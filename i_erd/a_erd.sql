...
/*	: Entity Relationship Diagram
    ...
    > 데이터(엔티티)와 데이터 간의 관계를 시작적으로 표현한 도표
    
    
    1. 사용 목적
    - 실무에서 DB 설계전, 정보 분석과 설계 방향을 명확하게 정리
    - 팀원 간 효율적인 의사소통 도구
    ...
    cf) 관계 유형
	- 1:1(일대일) - 한 엔티티가 다른 하나와만 관계 (사람 - 여귄)
    - 1:N(일대다) - 한 엔티티가 여러 개와 관계 (교수 - 강의, 학생 - 수강 내역, 게시글 - 댓글)
    - N:M(다대다) - 여러 엔티티가 서로 다수와 관계 (학생 - 과목: 중간 테이블 필요)
    ...
    구상 내역 테이블) 자식 테이블 - 수강 ID PK, 학번 FK, 강의명 ... 등
    >> 관계: 1명의 학생은 여러 개의 수강 내역을 가질수 있음 (1:N 관계)
	...
	>> 중간 테이블: 수강(student_subject)
        ...
        - 수강 날짜 등 추가 속성 정의 가능
*/
create database if not exists `comopsite`;
USE `comopsite`;
...
create table course_enrollment( # 강의 등록 테이블 (수강)
    ...
    # >> student_id와 student_id의 조합이 비워질 수 없으며, 둘 다 합쳐서 유일한 값이 되어야 함
