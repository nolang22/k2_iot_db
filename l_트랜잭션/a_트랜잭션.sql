...
/*	: 데이터베이스에서 하나의 논리적인 작업 단위를 의미
	- 여러 개의 작업(명렁어)이 한 번에 처리되어야 할 때 사용
...
### 3. 트랜잭션 4가지 특징 (ACID) ###
    ...
    Isolation (고립성): 동시에 여러 트랜잭션이 실행되어도, 서로 간섭하지 않음
    Durability (지속성): 트랜잭션 성공 시, 결과는 영구적으로 보존
    
### 4. 트랜잭션 실행 흐름 ###
	1) 트랜잭션 시작 - START TRANSACTION;
    2) SQL 명령어 실해 - INSERT, UPDATE, DELETE 등
    3) 문제 없이 완료되면 - COMMIT;
    4) 중간에 오류 발생 또는 취소 시 - ROLLBACK;
    
    start tansaction;
    
    # start 이후 불럭 내의 명렁어는 '하나의 명령어처럼 처리'
...
begin
    ...
    if from_balance >= 10000 then
        ...
        values
			(
				(select account_id from accounts where account_holder = '김지지'),
				(select account_id from accounts where account_holder = '김동동'),
                10000
            );
		commit;
	else
